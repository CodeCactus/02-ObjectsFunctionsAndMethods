...
def better_draw_circles(point, radius, number,drift):
    """
...
def even_better_draw_circles(point, radius, number, colur, thiccness):
    """
    Starts out the same as the   draw_circles   function defined ABOVE.
    You Will make it an IMPROVED, MORE POWERFUL function per the above _TODO_.
    """
    turtle = rg.SimpleTurtle()
    turtle.pen=rg.Pen(colur,thiccness)
    turtle.pen_up()
    turtle.go_to(point)
    turtle.set_heading(0)  # Point "east" (towards the right)

    for k in range(0, number):  # k becomes 1, 2, 3, ... 10

        color = [colur, 'red', 'orange', 'yellow', 'green', 'blue', 'indigo', 'violet']
        colors = color * (1+int((k+1)/7))
        turtle.pen = rg.Pen(colors[k], thiccness)
        turtle.pen_up()
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(radius)
